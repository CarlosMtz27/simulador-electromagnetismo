...
export class RenderizadorCelular {
   ...
   static animarHumoYFuego(ctx, cx, cy, humoObj) {
    ...
    humoObj.chispas.forEach(c => {
        c.y -= c.vel;
        if (c.y < -70) c.y = 0;
        const brightness = 0.6 + 0.4 * Math.sin(humoObj.angulo * 3 + c.fase);
        ctx.fillStyle = brightness > 0.8 ? '#FFEE88' : '#FFFFFF';
    ...
    for (let i = 0; i < 4; i++) {
        ...
        ctx.beginPath();
        ctx.arc(ox, -20 - (i * 25), 10 + (i * 5), 0, Math.PI * 2);
        ctx.fill();
    }
}
}